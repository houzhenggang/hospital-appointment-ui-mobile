...
<template>
  <div id="personsigning">
    ...
    <van-uploader :after-read="afterRead" class="uploader" v-if="false">
    ...
    <van-cell-group class="content">
      ...
      <van-field v-model="formData.idCard" clearable label="身份证" placeholder="请输入您的居民身份证" />

      <van-field v-model="formData.sexName" label="性别" placeholder="请选择您的性别" readonly @click="()=>{this.showList.gendershow = true}" @click-right-icon="()=>{this.showList.gendershow = true}" />

      <van-field v-model="formData.birthDate" label="生日" placeholder="请选择您的出生日期" readonly @click="opendatepicker" @click-right-icon="opendatepicker" />

      <van-field v-model="formData.nationalName" label="民族" placeholder="请选择您的民族" readonly @click="()=>{this.showList.nationshow = true}" @click-right-icon="()=>{this.showList.nationshow = true}" />
    ...
    <van-actionsheet v-model="showList.gendershow" :actions="dictMap.SexTypeList" cancel-text="取消" @select="ongenderSelect" @cancel="showList.gendershow = false" />
    ...
    <van-popup v-model="showList.nationshow" position="bottom">
      <van-picker show-toolbar :columns="dictMap.nationalList" @confirm="nationConfirm" @cancel="() => {this.showList.nationshow = false}" />
    </van-popup>
...
export default {
  ...
  methods: {
    ...
    // 选择民族
    nationConfirm (row) {
      this.formData.national = row.value
      this.formData.nationalName = row.text
      this.showList.nationshow = false
    },
    // 性别
    ongenderSelect (row) {
      this.showList.gendershow = false
      this.formData.sex = row.value
      this.formData.sexName = row.name
    },
    // 选择生日
    opendatepicker () {
    ...
    submitForm () {
      for (let index in this.formData) {
        if (this.formData[index] === '' && index !== 'idCardFront') {
