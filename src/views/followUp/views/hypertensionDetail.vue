<template>
  <div>
    <zForm @zFormFn="zFormFn" :options="options" v-model="formData">
    </zForm>
  </div>
</template>
...
<script>
import {
  getHypertensionFormById
} from '@/api/followUpHypertension'
import {
  hypertensionDetailOption
} from './const/hypertensionDetail'
...
export default {
  ...
  data () {
    return {
      options: hypertensionDetailOption,
      formData: {},
      hospitalId: '',
      teamId: '',
      teamIndex: 13,
  ...
  methods: {
    getFormData (id) {
      if (this.status === 'correlation') {
        ...
        getHypertensionFormById(id).then(({ data }) => {
