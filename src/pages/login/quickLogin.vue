...
export default {
  ...
  mounted () {
    ...
    window.getAccessTokenCallback = (params) => {
      params = JSON.parse(params)
      if (params.success) {
        this.accessToken = params.data.accessToken
        // 调起获取用户授权信息接口
        var userInfoParams = '{"accessToken":"' + this.accessToken + '","appId":"' + this.appId + '"}'
        // 调起获取用户授权信息，接口（首次展示用户授权界面）
        this.getUserInfoByToken(userInfoParams)
      } else {
        this.$message.error(`获取accessToken失败，原因：${params.msg}`)
      }
    }
    window.getUserInfoByTokenCallback = (params) => {
      params = JSON.parse(params) // jsonStr 转jsonObject
      if (params.success) {
        ...
        this.$message.error(`获取用户授权信息失败，原因：${params.msg}`)
      }
    }
  }
}
